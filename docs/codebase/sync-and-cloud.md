...
A chunk carries three entities in typed arrays — `sessions`, `observations`, `prompts` — and everything else only inside `mutations`. `WriteChunk` materializes the typed arrays row by row and then every mutation whose row those arrays did not already cover, so `relation`, all seven engram-projects entities, and any session, observation or prompt the arrays left out reach `cloud_mutations`. That table is what `ListMutationsSince` serves, so an entity missing from it is invisible to every pulling replica while the pushing client still acks the chunk — a silent, reason-code-less hole.

**The two halves of a chunk are not the same set, and the dedupe is per row.** The typed arrays hold what a timestamp window over the local tables selected (`filterNewData`); the mutation array holds what the journal owes the server. A session whose row predates the window travels only as a mutation, so treating the array as a superset of it — deduplicating by entity rather than by `(entity, key)` — drops exactly those rows. `typedChunkRows` indexes what this chunk's arrays carry and nothing else is skipped; mutations keep the array's own order, which is the journal's `seq` order, so the `cloud_mutations` seq a replica replays preserves the order the writes happened in.

`WriteChunk` only covers chunks written under the rule in force when they landed. Everything an older client pushed still sits in `cloud_chunks` alone, and no client can re-push a chunk the server already holds. `CloudStore.MaterializeChunkMutations` walks the stored chunks of a project and applies the same per-row rule to them, writing the missing mutations into `cloud_mutations`; `cloud serve` runs it over every project at start, and `engram cloud repair materialize-chunks [--project <name>] (--dry-run|--apply)` runs it on demand. It is idempotent, keyed on entity, key and op — never on the payload, so a later push carrying a newer payload stays a genuine update instead of being treated as a duplicate. The report carries `materialized_by_entity`, and the server start logs it: the count alone says how big the backlog was, the breakdown says which rule dropped it.
