...
│   ├── mcp/mcp.go                  # MCP stdio server (39 tools across 4 profiles)
...
- Cloud runtime (`engram cloud serve`) exposes `GET /health`, `GET /version`, `GET /sync/pull`, `GET /sync/pull/{chunkID}`, `POST /sync/push`, and `/dashboard/*`.
...
- [Dev Environment](DEV-ENVIRONMENT.md) — the isolated Docker stack, `make` targets, and the `scripts/dev/` scripts
