...
| ------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| Claude Code   | `claude plugin marketplace add HoracioEspinosa/engram && claude plugin install engram` | [Details](#claude-code)                            |
...
`engram setup pi` runs `pi install npm:@cloudfrog/memory@0.1.10` and `pi install npm:pi-mcp-adapter`, then ensures Pi settings contain both packages and writes `mcpServers.engram` in the Pi agent MCP config when no Engram server is already configured. Existing `mcpServers.engram` entries are preserved.
...
```bash
pi install npm:@cloudfrog/memory@0.1.10
...
`ENGRAM_URL` tells the `@cloudfrog/memory` Pi extension to use an already-running `engram serve` instance instead of auto-starting one. This is standard shell syntax: `KEY=value command`. The URL is the HTTP REST API base; it is not an MCP endpoint.
...
```bash
claude plugin marketplace add HoracioEspinosa/engram
...
- Best-effort installs the Codex plugin with `codex plugin marketplace add HoracioEspinosa/engram --ref main` and `codex plugin add engram@engram`
