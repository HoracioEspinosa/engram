...
func TestInstallPiInstallsPackagesAndWritesConfig(t *testing.T) {
	...
	wantCommands := []string{"pi install npm:@cloudfrog/memory@0.1.10", "pi install npm:pi-mcp-adapter"}
	...
	for _, pkg := range []string{"npm:@cloudfrog/memory@0.1.10", "npm:pi-mcp-adapter"} {
...
func TestInstallPiPreservesExistingEngramMCPServer(t *testing.T) {
	...
	if !strings.Contains(string(settingsRaw), "npm:existing") || !strings.Contains(string(settingsRaw), "npm:@cloudfrog/memory@0.1.10") || !strings.Contains(string(settingsRaw), "npm:pi-mcp-adapter") {
...
func TestInstallPiCommandFailure(t *testing.T) {
	...
	_, err := Install("pi")
	if err == nil || !strings.Contains(err.Error(), "install npm:@cloudfrog/memory@0.1.10") {
