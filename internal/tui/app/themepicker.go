...
var themePickerKeys = struct {
	Open, Apply, Reload, Close key.Binding
}{
	Open: key.NewBinding(
		key.WithKeys("ctrl+t"),
	...
	Apply: key.NewBinding(
		key.WithKeys("enter"),
		key.WithHelp("enter", "use this theme"),
	),
	Reload: key.NewBinding(
		key.WithKeys("R"),
		key.WithHelp("R", "reload from the store"),
	),
...
func (m Model) updateThemePicker(msg tea.KeyMsg) (bool, Model, tea.Cmd) {
	if !m.themePicker.open {
		if !key.Matches(msg, themePickerKeys.Open) || m.showHelp {
			return false, m, nil
		}
...
func (m Model) viewThemePicker() string {
	...
	b.WriteString("\n" + m.styles.Help.Render(
		"  j/k preview • enter use • / filter • R reload • esc cancel"))
	return b.String()
}
