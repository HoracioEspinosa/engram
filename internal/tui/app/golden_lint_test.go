...
// The golden files freeze what the screens look like. A diff against them
// catches a render that changed; it says nothing about a render that was
// always wrong, and `-update` turns any such render into the new truth by
// definition.
//
// This is the other half: the properties a frozen frame has to have whatever
// it says. It runs against the files on disk and against the very scenes the
// suite renders, so a regeneration that freezes a broken screen fails on the
// same run that wrote it rather than on a review that has to spot it in 1500
// lines of diff.
//
// Every rule is structural — a width, a separation, a marker — because a rule
// about content would only be the golden file again, spelled twice. The rules
// report their findings rather than failing directly, so the suite can hand
// them a broken frame and require that they notice.
...
func lintFrozenTabBar(scene frozenScene, width int) (problems []string, hasTabBar bool) {
	...
	bracketed := 0
	for i, span := range SlotSpans(row) {
...
// none. The bar sits on the frame's second row: the first is the app frame's
// own top padding.
...
// TestGoldenScreensAreStructurallySound lints the frozen documents.
func TestGoldenScreensAreStructurallySound(t *testing.T) {
...
// TestRenderedScreensAreStructurallySound runs the same rules against the
// scenes as they are rendered now, not as they were frozen.
//
// This is the half that makes `-update` safe: the document it writes is this
// render, so a regeneration that would freeze a broken screen fails on the
// same run rather than being reviewed as a 1500-line diff nobody can read
// structurally.
func TestRenderedScreensAreStructurallySound(t *testing.T) {
...
// TestTheGoldenLintCatchesEachFailure hands the rules a frame broken one way
// at a time.
//
// A lint that reports nothing passes on every input, including the ones it
// exists to reject, and it would do that silently for as long as the golden
// files happened to be sound. Each case here is a screen that has actually
// gone wrong at some point in this workspace's history.
func TestTheGoldenLintCatchesEachFailure(t *testing.T) {
	sound := frozenScene{name: "sound", lines: []string{
		"",
	...
	cases := map[string]frozenScene{
		...
		"a rune cut in half": {name: "replacement", lines: []string{
			"", "  0 Home  [1 Memory]  2 Tasks", "", "  a row\uFFFD",
			"  j/k move                    koi-pond",
		}},
		"a column run into an elision": {name: "glued", lines: []string{
			"", "  0 Home  [1 Memory]  2 Tasks", "", "  a long title…7 obs",
			"  j/k move                    koi-pond",
		}},
		"two tab bar slots run together": {name: "collapsed", lines: []string{
			"", "  0 Home  [1 Memory] 2 Tasks", "", "  a row",
			"  j/k move                    koi-pond",
		}},
		"no active tab bar slot": {name: "unbracketed", lines: []string{
			"", "  0 Home  1 Memory  2 Tasks", "", "  a row",
			"  j/k move                    koi-pond",
		}},
		"no status bar": {name: "bareless", lines: []string{
			"", "  0 Home  [1 Memory]  2 Tasks", "", "  a row",
		}},
		"the status bar's groups run together": {name: "squeezed", lines: []string{
			"", "  0 Home  [1 Memory]  2 Tasks", "", "  a row",
			"  j/k movekoi-pond",
		}},
		"nothing rendered at all": {name: "empty"},
	}
