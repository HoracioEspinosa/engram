...
// TestCmdTUIResolvesThemePrecedence pins the TUI theme's precedence chain
// — flag, then ENGRAM_TUI_THEME, then tui.theme in <data-dir>/config.json,
// then catppuccin-mocha — the same shape TestCmdTUIResolvesProjectPrecedence
// already pins for --project.
//
// Unlike TestNoTwoDistinctRolesShareAColour in internal/tui/theme, there is
// no pre-existing bug this test reproduces — it was written alongside the
